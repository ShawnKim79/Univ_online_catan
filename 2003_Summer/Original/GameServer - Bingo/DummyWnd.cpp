...
int CDummyWnd::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	// 트레이 아이콘 생성
	if(!m_TrayIcon.Create(this, WM_TRAYNOTIFY, "Binogo 게임 서버", LoadIcon(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDR_MAINFRAME)), IDC_TRAYICON))
	{
		return -1;
	}

//	m_Console.SetConsoleTitle("Bingo Server...");
	...
	m_pLogDialog->Create(IDD_LOG_DIALOG, this);
	...
	{
		m_pLogDialog->AddLogString("서버 소켓 초기화에 실패했습니다.");
		return 0;
	}
...
{
	m_pLogDialog->AddLogString("*** InitServer ***");
...
void CDummyWnd::OnStartserver() 
{
	...
	{
		m_pLogDialog->AddLogString("채널 서버 접속에 실패했습니다.");
		return;
	}

	m_pLogDialog->AddLogString("*** Succeeded to Connect to Channel Server ***");

	WSAAsyncSelect(m_ClientSocket.m_Socket, m_hWnd, WM_SOCKETMSGCHSERVER, FD_READ | FD_CLOSE);

	m_pLogDialog->AddLogString("*** Start Listen ***");
...
{
	m_pLogDialog->AddLogString("*** Stop Server ***\r\n");
...
long CDummyWnd::OnSocketMsgChServer(WPARAM wParam, LPARAM lParam)
{
	...
	if(WSAGETSELECTEVENT(lParam) == FD_READ)
	{
		...
		{	
			...
			//m_pLogDialog->AddLogString("nRead2(%d) < cmnpck.length(%d)", nRead2, cmnpck.length);
		}
...
void CDummyWnd::SendAllUsersInfo(CMessage *pMsg)
{
	...
	g_Critical.Lock();
	m_pLogDialog->AddLogString("SendAllUsersInfo() : Lock()");
	...
	g_Critical.Unlock();
	m_pLogDialog->AddLogString("SendAllUsersInfo() : Unlock() *");
...
void CDummyWnd::SendAllRoomsInfo(CMessage *pMsg)
{
	...
	g_Critical.Lock();
	m_pLogDialog->AddLogString("SendAllRoomsInfo() : Lock()");
...
//	m_pLogDialog->AddLogString("Sending RoomInfo... Total : %d, Sent : %d", nTotalLen, nSent);

	g_Critical.Unlock();
	m_pLogDialog->AddLogString("SendAllRoomsInfo() : Unlock() *");
...
{
	m_pLogDialog->AddLogString("*** MakeRoom ***");
	...
	g_Critical.Lock();
	m_pLogDialog->AddLogString("MakeRoom() : Lock()");
	...
	{		
		if(lproom->present == 0) // 아무도 입장하지 않았으면
		{
			...
			m_pLogDialog->AddLogString("::: GSM_OWNTHISROOM (id : %s)", (LPCTSTR)lpuser->id);
...
void CDummyWnd::JoinRoom(CMessage *pMsg)
{
	...
	g_Critical.Lock();
	m_pLogDialog->AddLogString("JoinRoom() : Lock()");
	...
	if(lproom != NULL)
	{
		...
		else if(lproom->present < lproom->maximum)
		{
			...
			m_pLogDialog->AddLogString("::: GSM_JOINTHISROOM (id : %s, compet id : %s)", (LPCTSTR)lpuser->id, (LPCTSTR)lproom->users[0]->id);
...
{
	m_pLogDialog->AddLogString("*** GetoutRoom ***");

	g_Critical.Lock();
	m_pLogDialog->AddLogString("GetoutRoom() : Lock()");
	...
	if(lproom != NULL)
	{
		...
		else
		{
			...
			m_pLogDialog->AddLogString("::: GSM_OWNTHISROOM (id : %s)", (LPCTSTR)lproom->users[0]->id);
		}
	...
	g_Critical.Unlock();
	m_pLogDialog->AddLogString("GetoutRoom() : Unlock() *");
}
...
void CDummyWnd::NotifyAllUpdate(LPUSERINFO lpuserinfo, LPROOM lproom)
{
	...
	g_Critical.Lock();
	m_pLogDialog->AddLogString("NotifyAllUpdate() : Lock()");
	...
	for(; iter != m_ClientManager.clientlist.end() ; iter++)
	{
		...
		if(lpuser->channelno == lproom->channelno)
		{
			...
			m_pLogDialog->AddLogString("AllUpdate : %s", (LPCTSTR)lpuser->id);
		}
	}
...
//	m_pLogDialog->AddLogString("******");
	...
	for(; iter != m_ClientManager.clientlist.end() ; iter++)
	{
		...
		{
			int nResult = m_ClientSocket.SendMsg(lpuser->socket, (char *)Send.GetDataPtr(), Send.GetLength());
//			m_pLogDialog->AddLogString1("%d", lpuser->socket);
		}
	}
...
void CDummyWnd::UpdateChatMessage(CMessage *pMsg)
{
	...
	g_Critical.Lock();
	m_pLogDialog->AddLogString("UpdateChatMessage() : Lock()");
...
// 다시 한번 입장 가능한지 검사
	...
	else
	{
		...
		strQuery.Format("SELECT userid, name, alias, score2, playcount2, victorycount2, losscount2, drawcount2 \
		...
		if(lpuser->roomno == -1)
		{
			...
			for(; iter != lpchannel->roomlist.end() ; iter++)
			{
				...
				if(lproom->present == 0)
				{
					...
					m_pLogDialog->AddLogString("::: GSM_OWNTHISROOM (id : %s)", (LPCTSTR)lprequester->id);
					...
					g_Critical.Unlock();
					m_pLogDialog->AddLogString("SendReqNewGame() : Unlock() *");
