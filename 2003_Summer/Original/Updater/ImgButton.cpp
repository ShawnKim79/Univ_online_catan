...
void CImgButton::DrawItem(LPDRAWITEMSTRUCT lpDIS) 
{
	...
	else // 이미지 등록이 없을 경우
	{
		...
		switch(m_nBtnState)
		{
			case BS_NORMAL : pDC->Draw3dRect(0, 0, lpDIS->rcItem.right, lpDIS->rcItem.bottom, GetSysColor(COLOR_3DSHADOW), GetSysColor(COLOR_3DSHADOW));
				pDC->DrawText(caption, &lpDIS->rcItem, DT_SINGLELINE|DT_VCENTER|DT_CENTER);
				break;

			case BS_OVER : pDC->Draw3dRect(0, 0, lpDIS->rcItem.right, lpDIS->rcItem.bottom, GetSysColor(COLOR_3DHILIGHT), GetSysColor(COLOR_3DDKSHADOW));
				pDC->DrawText(caption, &lpDIS->rcItem, DT_SINGLELINE|DT_VCENTER|DT_CENTER);
				break;

			case BS_CLICK : 

				pDC->Draw3dRect(0, 0, lpDIS->rcItem.right, lpDIS->rcItem.bottom, GetSysColor(COLOR_3DDKSHADOW), GetSysColor(COLOR_3DHILIGHT));
