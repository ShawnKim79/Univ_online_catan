...
extern "C" __declspec(dllimport) cOmokManage *getGameObject();
...
BOOL CLobbyDlg::ConnectToChannelServer()
{
	...
	if(!m_ClientSocket.Connect("218.53.191.241", 11000))
	{
...
void CLobbyDlg::GetCompetitorData(CMessage *pMsg)
{
	...
	if(m_bOwner)
	{
		m_pOmok->SetcompetitorInfo(m_CompetInfo);
...
void CLobbyDlg::UpdateAllInfo(CMessage *pMsg)
{
	...
	{
		m_bReadyGame = FALSE;
		::SendMessage(m_hOmokWnd, WM_COMMAND, IDC_OUTPUT, 0);
	}
...
void CLobbyDlg::StartGame(BOOL bServer)
{
	...
	if(m_pOmok == NULL)
	{
		m_pOmok = getGameObject(); // 오목 윈도우 객체 생성
		m_hOmokWnd = m_pOmok->createOmokWnd(m_hMyWnd);
		m_pOmok->SetParentHandle(m_hMyWnd);

		m_Console.writeConsol("\n\n\n*** First ***\n");
	}
 
	m_Console.writeConsol("\n\n\n*** Second ***\n");
	::ShowWindow(m_hOmokWnd , SW_SHOW);

	m_pOmok->SetMyInfo(m_MyInfo);

	if(bServer)
	{
		m_bOwner = TRUE;

		::SendMessage(m_hOmokWnd, WM_COMMAND, IDC_SERVER, m_bOfferGame);

		m_bOfferGame = FALSE;

		m_pOmok->SetNotice("", (LPBYTE)&cf);
		str.Format("▷ %d 번방 입장\n", m_CompetInfo.roomno);
	}
	...
	else
	{
		...
		m_pOmok->SetNotice((LPSTR)(LPCTSTR)str, (LPBYTE)&cf);

		::SendMessage(m_hOmokWnd, WM_COMMAND, IDC_CLIENT, 0);
...
{
	m_Console.writeConsol("OnGameResult\n");
