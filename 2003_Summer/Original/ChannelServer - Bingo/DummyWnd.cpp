...
int CDummyWnd::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	// 트레이 아이콘 생성
	if(!m_TrayIcon.Create(this, WM_TRAYNOTIFY, "Bingo 최널 서버", LoadIcon(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDR_MAINFRAME)), IDC_TRAYICON))
	{
		return -1;
	}
...
*/
	...
	if(!m_ServerSocket.Initialize(m_nPortServer))
	{
	...
	// 클라이언트와 통신할 소켓 초기화
	if(!m_ClientSocket.Initialize(m_nPortClient))
	{
		AfxMessageBox("서버 소켓 초기화에 실패했습니다.");
		return 0;
	}
